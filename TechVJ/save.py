...
async def batch_link(_, message):
    ...
    start = await Client.ask(message.chat.id, text="Please send the start link.")
    start_id = start.text
    s = start_id.split("/")[-1]
    cs = int(s)
    
    last = await Client.ask(message.chat.id, text="Please send the end link.")
    last_id = last.text
    l = last_id.split("/")[-1]
    cl = int(l)

    if cl - cs > 1000:
        await Client.send_message(message.chat.id, "Only 1000 messages allowed in batch size... Make sure your start and end message have difference less than 1000")
        return
    ...
    try:     
        ...
        if data and data.get("session"):
            ...
            try:
                ...
                return await Client.send_message(message.chat.id, "Please generate a new session.")
        else:
            await Client.send_message(message.chat.id, "Please generate a session first.")
            ...
            for i in range(int(s), int(l)):
                if user_id in users_loop and users_loop[user_id]:
                    ...
                    try:
                        ...
                        sleep_msg = Client.send_message(message.chat.id, "Sleeping for 10 seconds to avoid flood...")
